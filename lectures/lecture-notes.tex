...
\begin{example}[Examples of linear equations]
  ...
  \begin{itemize}
    ...
    it makes sense that the dimension is $2$. Etc.
  \end{itemize}
...
\begin{center}
  \begin{tcolorbox}[width=0.9\textwidth, colback=white, colframe=black]
    \textit{\textbf{The do nexus questions this lecture:}
      \vspace{.5em}
      \begin{enumerate}
...
Apply operation 3 again: This time, replace $E_{3}$ with $E_{3}+2 E_{1}$:
\begin{equation*}
  \left\{
    \begin{alignedat}{4}
      &x &{}\;-\; y &{}\;+\; &z{}&= &0 \\
...
\begin{equation*}
  ...
  \right.
\end{equation*}
Apply operation $2$ by dividing equation equation by $3$:
\begin{equation*}
  \left\{
    \begin{alignedat}{3}
      x \quad& &  & =&{} -7/3 \\
