...
\begin{center}
  \begin{tcolorbox}[width=0.9\textwidth, colback=white, colframe=black]
    \textit{\textbf{The nexus question of this lecture:} What do solutions to
      systems of equations look like?}
      % \vspace{.5em}
      % \begin{enumerate}
...
\begin{enumerate}
  \item A slope $m$ which determines the direction of hte line
...
\end{enumerate}
Indeed, a plane $\P$ consists of the set of points $(x,y,z)$ satisfying the
following equation:
...
\begin{equation*}
  Ax+By+Cz=D
\end{equation*}
where $D = Ax_{0}+By_{0}+Cz_{0}$. This is a linear equation. The solutions to
...
\begin{example}[A system with three variables]
  ...
  unique point of intersection of hte planes.

  Here's the desmos link to see the plots of these three planes, if you want
  to play around with it.
  \begin{center}
    \url{https://www.desmos.com/3d/gpgtw2rjaf}
  \end{center}

  Of course ther are other ways that three planes could have intersected. For
  example, two of the planes might be parallel, like the following picture, in
  which case the system will have no solutions:
  \begin{center}
    \includegraphics[scale=.2]{images/two-parallel-planes}
  \end{center}
